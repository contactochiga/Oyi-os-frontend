  ...
  async chat(message: string, context?: Record<string, any>): Promise<AiChatResponse> {
    try {
      const unified = await oyiService.chat({
        ...
        ...(context?.page_launch_context ? { page_launch_context: context.page_launch_context } : {}),
        ...(context?.selected_ui_object ? { selected_ui_object: context.selected_ui_object } : {}),
        ...(context?.current_turn_hints ? { current_turn_hints: context.current_turn_hints } : {}),
        ...(context?.authorised_scope ? { authorised_scope: context.authorised_scope } : {}),
