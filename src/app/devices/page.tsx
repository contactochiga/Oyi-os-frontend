...
/**
 * ✅ THE ONLY COMMAND MAP WE USE (copied from your working page)
 * - 1-gang:  "switch"
 * - 2/3-gang: "switch_1", "switch_2", "switch_3"
 */
function gangCommandKey(gangCount: 1 | 2 | 3, gangIndex: number) {
  if (gangCount === 1) return "switch";
  return `switch_${gangIndex + 1}`;
}
...
export default function DevicesPage() {
  ...
  // ✅ Master ring -> toggle ALL gangs (using the working command keys only)
  async function toggleMaster(device: AnyDevice) {
    ...
    try {
      ...
      // ✅ command payload exactly like the working page expects
      const cmd: Record<string, any> = {};
      for (let gi = 0; gi < gangCount; gi++) {
        cmd[gangCommandKey(gangCount, gi)] = next;
      }
  ...
  // ✅ In modal: toggle a single gang (working keys only)
  async function toggleGang(device: AnyDevice, gangIndex: number, next: boolean) {
    ...
    try {
      ...
      const key = gangCommandKey(gangCount, gangIndex);
      ...
      await deviceService.commandDevice(sid, { [key]: next });
    } catch (e: any) {
      setErr(e?.response?.data?.error || e?.message || "Command failed (device may be offline)");
    } finally {
      setBusyId(null);
