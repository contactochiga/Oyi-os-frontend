...
export default function HomePage() {
  ...
  async function handleSend(text?: string) {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { id: createId(), role: "assistant", content: reply, panel, panelTag: panel, time: now }]);
      if (panel === "devices") {
      ...
      setTimeout(() => { if (isAtBottom()) chatRef.current?.scrollTo({ top: chatRef.current.scrollHeight, behavior: "smooth" }); }, 80);
    } catch (err) {
      setMessages((prev) => [...prev, { id: createId(), role: "assistant", content: "Sorry, AI unreachable.", time: now }]);
    }
  }
      ...
      <main className="fixed inset-0 flex flex-col">
            ...
            {messages.map((m) => (
              <div key={m.id} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                <div className="max-w-[80%]">
                  {m.content && <div className={`px-4 py-2 rounded-2xl ${m.role === "user" ? "bg-blue-600 text-white" : "bg-gray-900 text-gray-100"}`}>{m.content}</div>}
                  {m.panel && <div className="mt-2">{/* Render a panel component based on tag */}</div>}
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="w-full px-4 z-40 pointer-events-none">
          <div className="max-w-3xl mx-auto pointer-events-auto">
            <DynamicSuggestionCard suggestions={[
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-gray-900 border-t border-gray-700">
          <ChatFooter input={input} setInput={setInput} onSend={() => handleSend(undefined)} />
        </div>
      </main>
    </LayoutWrapper>
  );
