...
export default function HomePage() {
  ...
  const pendingVisitors = useMemo(() => {
    // pending for gate flow = "approved" (ready to enter) OR anything not denied/exited
    return visitors.filter((v) => {
      const s = String(v.status || "").toLowerCase();
      return s === "approved";
    }).length;
  ...
  const unreadCommunity = useMemo(() => {
    // Prefer notifications typed as community; fallback 0
  ...
  const visitorsByDay = useMemo(() => {
    return sumByWeekday(
      visitors.map((v) => ({ created_at: v.created_at }))
    );
  }, [visitors]);
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                    ...
                    <StatCard
                      ...
                      sub={pendingVisitors ? `${pendingVisitors} approved` : "No approved visitors"}
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                    ...
                    <StatCard
                      ...
                      sub="Chat control"
                      onClick={() => {
                        // scroll down to chat area smoothly
                        bottomRef.current?.scrollIntoView({ block: "end", behavior: "smooth" });
                      }}
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            <Pie
                              ...
                              innerRadius={40}
                            >
                              {/* keep it consistent but not theme-breaking */}
                    ...
                    <PanelCard
                      ...
                      subtitle="Created visitor access (by weekday)"
                    >
                      <div className="h-[180px]">
                        <ResponsiveContainer width="100%" height="100%">
