                ...
                {twinRooms.length ? (
                  <div className="absolute inset-3 transition-transform duration-500 ease-out">
                    ...
                    {twinRooms.map((space) => {
                      ...
                      return (
                        <button key={space.id} type="button" onClick={() => router.push(`/room?roomId=${encodeURIComponent(space.id)}&roomName=${encodeURIComponent(space.name)}`)} className={cn("absolute rounded-[16px] border text-left transition-all duration-500 ease-out", active ? "z-10 scale-[1.035] border-sky-300/85 bg-sky-400/[0.13] shadow-[0_0_38px_rgba(0,122,255,0.58),inset_0_0_32px_rgba(0,122,255,0.22)]" : "scale-[0.985] border-slate-400/18 bg-white/[0.03] opacity-56 hover:scale-[1.01] hover:opacity-90")} style={{ left: `${space.x}%`, top: `${space.y}%`, width: `${space.w}%`, height: `${space.h}%` }}>
