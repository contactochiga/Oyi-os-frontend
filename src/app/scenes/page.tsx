          ...
          <div className="mx-auto max-w-[430px]">
            <header className="flex items-start justify-between gap-3">
              <div className="flex items-start gap-3">
                <HamburgerMenu />
                <div><h1 className="text-[30px] font-semibold leading-none tracking-[-0.055em]">Scenes</h1><p className="mt-2 text-[13px] text-white/52">Set the mood. Let your home respond.</p></div>
              </div>
              <MessagesInboxButton />
            </header>
...
function CreateSheet({ tab, initial, devices, onClose, onSaved }: { tab: Tab; initial?: ConsumerScene | ConsumerAutomation | null; devices: AnyDevice[]; onClose: () => void; onSaved: () => void }) {
  ...
  async function save() {
    ...
    try {
      if (tab === "scenes") {
        if (initial?.id) await sceneService.updateScene(initial.id, { name: name.trim(), actions });
        else await sceneService.createScene({ name: name.trim(), actions });
      } else {
