...
assert(/subtitle="Utility services for this home"/.test(servicesPage) && /stickyHeader/.test(servicesPage), "services page uses the stable compact home-utility header");
assert(/place-items-center/.test(servicesPage) && /amountInputRef/.test(servicesPage) && /quickAmounts/.test(servicesPage), "electricity purchase opens as a visible centered amount flow");
...
assert(!/document\.addEventListener\("scroll"/.test(bottomNav) && /VIEWPORT_RESIZE_GUARD_MS/.test(bottomNav), "bottom navigation uses one scroll source and ignores viewport resize noise");
